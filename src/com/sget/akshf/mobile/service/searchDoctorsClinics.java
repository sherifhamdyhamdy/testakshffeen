...
import com.sget.akshef.hibernate.service.SpecialistHasBranchService;
...
@Path("/searchDoctorsClinics")
public class searchDoctorsClinics {

	private Gson gson = new Gson();
	private SpecialistHasBranchService service = new SpecialistHasBranchService();
	@Context
	...
	public String searchDoctorsClinic( @FormParam("doctor_name") String doctorName, @FormParam("section_id") int sectionId, 
			@FormParam("country_id") int countryId, @FormParam("city_id") int cityId,@FormParam("district_id") int districtId,
			@FormParam("gender") int gender, @FormParam("degree_id") int degree, 
			@FormParam("orderByRate") int orderByRate, @FormParam("start") int start, @FormParam("limit") int limit) {
		...
		if (start >= 0 && limit > 0) {
			try {
				...
				List<SpecialistHasBranchBean> doctors = service.searchDoctorsClinics(search,url);
				if (doctors != null && doctors.size() > 0) {
					ResponseCollectFormatData<SpecialistHasBranchBean> respo = new ResponseCollectFormatData<SpecialistHasBranchBean>();
